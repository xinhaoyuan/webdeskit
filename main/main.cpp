...
static int register_native_method(const char *method_name, webdeskit_js_callback_f func);
...
webdeskit_host_interface_s host_interface = { register_native_method };
map<string, void *> plugin_handle_map;
map<string, webdeskit_plugin_interface_t> plugin_interface_map;
...
{
    webdeskit_plugin_init_f _init_f;
    webdeskit_plugin_interface_t interface;
    void *handle;
    ...
    _init_f = (webdeskit_plugin_init_f)dlsym(handle, "init");
    if (_init_f == NULL)
    {
...
main(int argc, char* argv[]) {
    ...
    /* Load home page */
    char __empty[] = "";
    char *home = getenv("WEBDESKIT_HOME");
    if (home == NULL) home = __empty;
    char *cwd = g_get_current_dir();
    char *path = home[0] == '/' ? home : g_build_filename(cwd, getenv("WEBDESKIT_HOME"), NULL);
